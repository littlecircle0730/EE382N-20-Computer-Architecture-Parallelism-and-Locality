...
class LEAST_RECENTLY_USED
{
  private:
    CACHE_TAG _tags[MAX_ASSOCIATIVITY];
    UINT32 _tagsLastIndex;
    UINT32 _keys[MAX_ASSOCIATIVITY];
    //UINT32 _nextReplaceIndex;

  public:
    LEAST_RECENTLY_USED(UINT32 associativity = MAX_ASSOCIATIVITY)
      : _tagsLastIndex(associativity - 1)
    {
        ASSERTX(associativity <= MAX_ASSOCIATIVITY);
        //_nextReplaceIndex = _tagsLastIndex;
    ...
    UINT32 Find(CACHE_TAG tag)
    {
        ...
        for (INT32 index = _tagsLastIndex; index >= 0; index--)
        {
            ...
            if(_tags[index] == tag)  
            { 
                for (INT32 idx = _tagsLastIndex; idx >= 0; idx--)
                    _keys[idx] = (_keys[idx] < _keys[index]) ? _keys[idx] : _keys[idx] - 1;

                _keys[index] = _tagsLastIndex;
                goto end;
            }
        }
        result = false;
    ...
    UINT32 Find(UINT32 *first, UINT32 *last, const UINT32 target)
    {
        UINT32 index = 0;
        while (first != last)
        {
    ...
    {
        // g++ -O3 too dumb to do CSE on following lines?!
        const UINT32 index = Find(_keys, _keys + _tagsLastIndex, 0);
...
#define CACHE_LEAST_RECENTLY_USED(MAX_SETS, MAX_ASSOCIATIVITY, ALLOCATION) CACHE<CACHE_SET::LEAST_RECENTLY_USED<MAX_ASSOCIATIVITY>, MAX_SETS, ALLOCATION>
